...
aws --endpoint-url=http://localhost:4566 dynamodb create-table --table-name sensorial.astronomy --attribute-definitions AttributeName=id,AttributeType=S --key-schema AttributeName=id,KeyType=HASH --provisioned-throughput ReadCapacityUnits=2,WriteCapacityUnits=2
